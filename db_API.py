...
def userValiadtion(cursor, email, password):
    r = cursor.execute(
        f"SELECT id, password, school, permission FROM user WHERE email='{email}';")
    r = r.fetchall()
    if len(r) == 0:
...
def getScore(cursor, user_id, categories):
    ...
    for category in categories:
        category_data = {}
        r = cursor.execute(
            f"SELECT * FROM {category}Score WHERE id={user_id};")
...
def setScore(cursor, user_id, category, data):
    ...
    if not exists:
        cursor.execute(
            f"INSERT INTO {category}Score ('id') VALUES ({user_id});")
...
def bestExport(cursor, category):
    ...
    data = sorted(data, key=lambda x: (x["score"], int(
        x["year"]), int(x["month"]), int(x["day"])))
    data.reverse()
    ...
    for i, student in enumerate(data):
        ...
        address, school = cursor.execute(
            f"SELECT address, school FROM user WHERE id={student_id}").fetchall()[0]
...
def scoreExport(cursor, category):
    ...
    schools = cursor.execute(
        f"SELECT school FROM user WHERE permission=1;").fetchall()
    ...
    for row in r.fetchall():
        ...
        school = cursor.execute(
            f"SELECT school FROM user WHERE id={row_id}").fetchall()[0][0]
...
@connect(db_path)
def addBest(cursor, table, user_id, name, surname, birthdate, score, grade):
    r = cursor.execute(
        f"INSERT INTO {table} \
...
@connect(db_path)
def addRow(cursor, table, user_id):
    cursor.execute(f"INSERT INTO {table} ('id') VALUES ({user_id})")


@connect(db_path)
def removeScoreRow(cursor, table, user_id):
