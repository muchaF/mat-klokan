...
class finalExport(file):
    ...
    def fill(self) -> None:
        for category in self.results:
            ...
            maxScore = len(self.results[category]['score'])
            for index, school in enumerate(self.results[category]['score']):
                ...
                for row in range(maxScore, -1, -1):
                    sheet[letter + str(maxScore - row + 6)] = self.results[category]['score'][school][str(row)]
