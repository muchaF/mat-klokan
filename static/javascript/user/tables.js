...
function updateTo(name) {
    pull(name).then((response) => {
        ...
        for (let player in response.best) {
            ...
            playerObject.innerHTML = "<td> <input placeholder='jméno' type='text' value=" + values.name + "> </td> \
            <td> <input placeholder='přijmeni' type='text' value=" + values.surname + "> </td> \
            <td> <input placeholder='třída' type='text' value=" + values.grade + "> </td> \
            <td class='sample'><input type='date' value=" + values.date + "> </td>\
            <td class='scinput'>\
                <input type='number' min='0' placeholder='body' value= " + values.score + ">\
                <img src='/static/img/svg/remove.svg' onclick='decrement(this)' >\
                <img src='/static/img/svg/add.svg' onclick='add(this)' class='plus'> </td> \
...
function addPlayer() {
    ...
    row.innerHTML = "<td><input type='text' placeholder='jméno'> </td> \
    <td> <input placeholder='přijmení' type='text'> </td> \
    <td> <input placeholder='třída' type='text'> </td> \
    <td class='sample'><input type='date' placeholder='datum'> </td> \
    <td class='scinput'> \
        <input placeholder='skóre' min='0' value='0' type='number'>\
        <img src='/static/img/svg/remove.svg' onclick='decrement(this)' >\
        <img src='/static/img/svg/add.svg' onclick='add(this)' class='plus'> </td> \
    <td> <img onclick='removeRow(this)' onload='updateCSS()' src='/static/img/svg/delete.svg'></td>"
